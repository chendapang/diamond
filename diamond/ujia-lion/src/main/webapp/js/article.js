layui.use(['paging', 'form','laytpl'], function() {
	...
	$('#add').on('click', function() {//因为添加按钮在列表外面，只会刷新一次，所以不需要动态绑定
		$.get('/page/article/article_edit', null, function(formHtml) {
	...
	function bindEvent(){
		$('[data-opt="edit"]').on('click', function() {
			var id = $(this).data("id");
			$.get('/page/article/article_edit', null, function(formHtml) {//获取表单模板
				
				  $.get("/article/doGet",{"id":id},function(result){//获取数据
					 var obj =  result.data;
		...
		$('[data-opt="del"]').on('click', function() {
			...
			popConfirm("确认删除？","删除后不可回复，请确认",function(){
				$.postJson("/article/doDelete",{"id":id},function(result){
					if(result.success){
						layer.msg("删除成功");
						paging.init();//刷新分页
					}else{
	...
	function popEditDialog(title,html){//弹出编辑对话框
		layer.open({
			...
			btn1: function(index) {//按钮1的回调 从1开始
				var formData = $("#articleForm").formToJson();		
	
				$.postJson("/article/doEdit",formData,function(result){
					if(result.success){
						layer.msg("保存成功");
...
function clearAuthorAvatar(){
	$("[name='authorAvatar']").val("");
	$("[name='authorAvatar_src']").attr("src","");
}
